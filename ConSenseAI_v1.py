...
def fact_check(tweet_text, tweet_id, context=None):
    ...
    for model in randomized_models:
    ...
    models_verdicts = ' '.join(f"\n\n🤖{model['name']}:\n {verdict[model['name']]}" for model in randomized_models)
    
    # summarize the verdicts by one of the models
    try:
        user_msg = f"Summarize the following responses that you just generated :\n{models_verdicts}\n\nProvide a concise summary of the overall consensus,\
            highlighting key points and any significant differences in the models' responses.\
            Don't do any additional searches or analysis, just summarize the responses you have already generated."
        verdict = run_model(system_prompt, user_msg, randomized_models[0], verdict)
        models_verdicts = verdict[randomized_models[0]['name']].strip()
        models_verdicts += '\n\nGenerated by:'
        models_verdicts += ' '.join(f"{model['name']}, " for model in randomized_models)
        models_verdicts += f'\nSummarized by: {randomized_models[0]["name"]}'
    except Exception as e:
